...
public function get_customer_purchase_data($id)
  {
...
public function get_customer_paid_data($id)
  {
  $tsale = $this->db->select("SUM(paidAmount) as ptotal")
                  ->FROM('sales')
                  ->WHERE('customerID',$id)
                  ->get()
                  ->row();
  ...
  $tvpaid = $this->db->select("SUM(totalamount) as total")
                    ...
                    ->get()
                    ->row();
  $query = $tsale->ptotal+$tvpaid->total;
  return $query;
}

public function get_customer_pdue_data($id)
  {
  $customer = $this->db->select("balance")
                  ->FROM('customers')
                  ->WHERE('customerID',$id)
                  ->get()
                  ->row();
                  
  $tsale = $this->db->select("SUM(dueamount) as ptotal")
                  ->FROM('sales')
                  ->WHERE('customerID',$id)
                  ->get()
                  ->row();
  ...
  $tvpaid = $this->db->select("SUM(totalamount) as total")
                    ...
                    ->get()
                    ->row();
  $query = (($customer->balance+$tsale->ptotal)-$tvpaid->total);
  return $query;
}
...
public function customer_fetch_data()
  {
  $query = $this->db->select('*')
                ->from('customers')
                ->get()
...
public function get_customer_group_data($id)
  {
  $query = $this->db->select('*')
                    ->from('customer_group')
                    ->where('cgid',$id)
...
public function get_customer_limit_data()
  ...
  foreach($customer as $value)
    {
    $tsale = $this->db->select("SUM(dueamount) as total")
                    ->FROM('sales')
...
public function get_purchse_product_data()
  {
  if($_SESSION['role'] > 2)
    {
    $query = $this->db->select('
...
public function get_dpurchse_product_data($sdate,$edate,$supid,$compid)
  {
  if($supid == 'All' && $compid == 'All')
    {
    $query = $this->db->select('
...
public function get_mpurchse_product_data($month,$year,$supid,$compid)
  {
  if($supid == 'All' && $compid == 'All')
    {
    $query = $this->db->select('
...
public function get_ypurchse_product_data($year,$supid,$compid)
  {
  if($supid == 'All' && $compid == 'All')
    {
    $query = $this->db->select('
...
public function get_product_stock_price($pid,$batch)
  {
  $query = $this->db->select('stock.totalPices,purchase_product.sprice')
                  ->from('stock')
                  ->join('purchase_product','purchase_product.batch = stock.batch','left')
                  ->where('stock.product',$pid)
